...
async def handle_code(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if compile_result.returncode == 0:
            ...
            await update.message.reply_text(
                "Code compiled successfully! Send your input (one value per line, e.g., '5\\n1 2 3 4 5\\n3'). Type 'none' if no input is needed."
            )
            return INPUT
        else:
...
async def handle_input(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if user_input.lower() == 'none':
            ...
            # Ensure input is passed as a single string with newlines
